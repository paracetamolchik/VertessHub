...
function new.Window(namewindow)
	...
	size(Sectionsframe, 0, 111, 0, 243)
	...
	size(MainSections, 0, 300, 0, 245)
	pos(MainSections, 0, 118, 0, 33)
	...
	Close.MouseButton1Click:Connect(function()
		...
		if timernotify <= 0 then
			notify.Visible = true
			t:Create(notify, info5, {Position = UDim2.new(1, -40, 1, -40)}):Play()
			wait(4)
			t:Create(notify, info9, {Position = UDim2.new(1, 160, 1, -40)}):Play()
			wait(0.8)
	...
	function Sections:Section(namesection)
		...
		function Buttons:Slider(textbtn, default, max, min, nums, callback)
			SectionPage.CanvasSize = SectionPage.CanvasSize + UDim2.new(0, 0, 0, 31)
		...
		function Buttons:Dropdown(textbtn, Buttons, callback)
			SectionPage.CanvasSize = SectionPage.CanvasSize + UDim2.new(0, 0, 0, 31)
			local opened = false
			local opening = false
			local sizeYframe = 0
			local frame1 = Instance.new("Frame", CSectionPage) borderpixel(frame1)
			size(frame1, 1, 0, 0, 26)
			tbackground(frame1)
			
			local btn = Instance.new("TextLabel", frame1) borderpixel(btn)
			text(btn, "")
			size(btn, 1, 0, 0, 26)
			corner(btn, 0, 7)
			stroke(btn, 0, 0, 150, 1)
			...
			local visualbtn = Instance.new("ImageLabel", btn) borderpixel(visualbtn)
			...
			local titlebtn = Instance.new("TextButton", btn) borderpixel(titlebtn)
			...
			local titletext = Instance.new("TextLabel", btn) borderpixel(titletext)
			text(titletext, Buttons[1])
			size(titletext, 1, 0, 1, 0)
			pos(titletext, 0, -25, 0, 0)			
			tbackground(titletext)
			textColor(titletext, 175, 175, 175)
			textFont(titletext, Enum.Font.Cartoon)
			textX(titletext, Enum.TextXAlignment.Right)
			textSize(titletext, 11)
			...
			for i,v in pairs(Buttons) do
				...
				btn1.MouseButton1Click:Connect(function()
					...
					local tween2=t:Create(frame1,info,{Size=UDim2.new(1,0,0,26)})
					tween2:Play()
					tween1:Play()
			...
			titlebtn.MouseButton1Click:Connect(function()
				if not opened and not opening then
					...
					local tween2=t:Create(frame1,info,{Size=UDim2.new(1,0,0,sizeYframe+26)})
					tween2:Play()
					tween1:Play()
					tween1.Completed:Wait()
					opening = false
			...
			return {
				Update = function(newtitle, newbuttons)
					...
					for i,v in pairs(ButtonsFrame:GetChildren()) do
						if v:IsA("TextButton") or v.Name ~= "Border" then
							v:Destroy()
						end
					end
					corner(ButtonsFrame, 0, 7)
					stroke(ButtonsFrame, 0, 0, 150, 1)
					for i,v in pairs(newbuttons) do
						sizeYframe = sizeYframe + 19
						if i > 1 then
							local border2 = Instance.new("Frame", ButtonsFrame) borderpixel(border2)
							size(border2, 1, 0, 0, 1)
							pos(border2, 0, 0, 0, (i-1)*19)
		...
		function Buttons:ColorPicker(title, color, callback)
			SectionPage.CanvasSize = SectionPage.CanvasSize + UDim2.new(0, 0, 0, 31)
			local rgb = {"R","G","B"}
			local hsv = {"H","S","V"}
			local textboxs = {}
			...
			size(frame1, 1, 0, 0, 26)
			tbackground(frame1)
			local btn = Instance.new("TextButton", frame1) borderpixel(btn)
			text(btn, "")
			size(btn, 1, 0, 0, 26)
			corner(btn, 0, 7)
			stroke(btn, 0, 0, 150, 1)
			...
			textFont(titletext, Enum.Font.Cartoon)
			textSize(titletext, 16)
			textX(titletext, Enum.TextXAlignment.Left)
			size(titletext, 1, -90, 1, 0)
			tbackground(titletext)
			
			local visualcolor = Instance.new("Frame", btn) borderpixel(visualcolor)
			backgroundcolor(visualcolor, color.R*255, color.G*255, color.B*255)
			size(visualcolor, 0, 30, 0, 18)
			pos(visualcolor, 1, -40, 0, 4)
			corner(visualcolor, 0, 6)
			stroke(visualcolor, 0, 0, 150, 1)
			
			local framePicker = Instance.new("ScrollingFrame", btn) borderpixel(framePicker)
			framePicker.CanvasSize = UDim2.new(0, 0, 0, 0)
			framePicker.Visible = false
			backgroundcolor(framePicker, 0, 0, 25)
			size(framePicker, 1, 0, 0, 0)
			pos(framePicker, 0, 0, 1, 1)
			corner(framePicker, 0, 7)
			stroke(framePicker, 0, 0, 150, 1)
			
			local PC = Instance.new("ImageLabel", framePicker) borderpixel(PC)
			backgroundcolor(PC, 255, 255, 255)
			pos(PC, 0, 8, 0, 8)
			size(PC, 0, 206, 0, 100)
			corner(PC, 0, 4)
			img(PC, "rbxassetid://133890899426165")
			
			local darkPC = Instance.new("Frame", framePicker) borderpixel(darkPC)
			backgroundcolor(darkPC, 255, 255, 255)
			pos(darkPC, 1, -28, 0, 8)
			size(darkPC, 0, 20, 0, 100)
			corner(darkPC, 0, 4)
			
			local gradientdarkPCC = Instance.new("UIGradient", darkPC)
			gradientdarkPCC.Rotation = 90
			gradientdarkPCC.Color = ColorSequence.new{
				ColorSequenceKeypoint.new(0, Color3.new(1, 1, 1)),
				ColorSequenceKeypoint.new(1, Color3.new(0, 0, 0))}
			
			local circlePC = Instance.new("Frame", PC) borderpixel(circlePC)
			tbackground(circlePC)
			size(circlePC, 0, 4, 0, 4)
			corner(circlePC, 1, 0)
			center(circlePC)
			stroke(circlePC, 0, 0, 0, 1)
			
			local circledarkPC = Instance.new("Frame", darkPC) borderpixel(circledarkPC)
			tbackground(circledarkPC)
			size(circledarkPC, 0, 4, 0, 4)
			corner(circledarkPC, 1, 0)
			center(circledarkPC)
			stroke(circledarkPC, 0, 0, 0, 1)
			
			for i = 1, 3 do
				local rgbtitle = Instance.new("TextLabel", framePicker) borderpixel(rgbtitle)
				text(rgbtitle, rgb[i]..": ")
				textColor(rgbtitle, 255, 255, 255)
				textFont(rgbtitle, Enum.Font.Cartoon)
				textSize(rgbtitle, 15)
				textX(rgbtitle, Enum.TextXAlignment.Left)
				size(rgbtitle, 0, 20, 0, 20)
				tbackground(rgbtitle)
				pos(rgbtitle, 0, 8+50*(i-1), 0, 114)
				
				local rgbTextBox = Instance.new("TextBox", framePicker) borderpixel(rgbTextBox)
				text(rgbTextBox, math.floor(color[rgb[i]]*255))
				textColor(rgbTextBox, 255, 255, 255)
				textFont(rgbTextBox, Enum.Font.Cartoon)
				textSize(rgbTextBox, 15)
				size(rgbTextBox, 0, 28, 0, 16)
				backgroundcolor(rgbTextBox, 0, 0, 0)
				stroke(rgbTextBox, 0, 0, 150, 1)
				pos(rgbTextBox, 0, 24+50*(i-1), 0, 115)
				corner(rgbTextBox, 0, 4)
				rgbTextBox:GetPropertyChangedSignal("Text"):Connect(function()
					if #rgbTextBox.Text > 3 then
						rgbTextBox.Text = rgbTextBox.Text:sub(1, 3)
					end
					if not tonumber(rgbTextBox.Text) then
						rgbTextBox.Text = rgbTextBox.text:gsub("%D+", "")
					end
				end)
				
				table.insert(textboxs, rgbTextBox)
				
				rgbTextBox.FocusLost:Connect(function()
					local text = tonumber(rgbTextBox.Text)
					if text > 255 then
						rgbTextBox.Text = "255"
						text = 255
					end
					if text then
						if rgb[i] == "R" then
							color = Color3.new(text/255, color.G, color.B)
						elseif rgb[i] == "G" then
							color = Color3.new(color.R, text/255, color.B)
						elseif rgb[i] == "B" then
							color = Color3.new(color.R, color.G, text/255)
						end
						h, s, v = color:ToHSV()
						circlePC.BackgroundColor3 = color
						circledarkPC.BackgroundColor3 = color
						visualcolor.BackgroundColor3 = color
						local PCSize = PC.AbsoluteSize
						local darkPCSize = darkPC.AbsoluteSize
						local posx = h * PCSize.X
						local posy = (1 - s) * PCSize.Y
						local posy2 = PCSize.Y - darkPCSize.Y
						circlePC.Position = UDim2.new(0, posx, 0, posy)
						circledarkPC.Position = UDim2.new(0, darkPCSize.X/2, 0, posy2)
						callback(color)
					end
				end)
			end
			
			local border2 = Instance.new("Frame", framePicker) borderpixel(border2)
			backgroundcolor(border2, 0, 0, 150)
			pos(border2, 0, 4, 0, 138)
			size(border2, 1, -8, 0, 1)
			...
			text(rainbowtoggle, "")
			pos(rainbowtoggle, 0, 8, 0, 145)
			size(rainbowtoggle, 0, 14, 0, 14)
			tbackground(rainbowtoggle)
			stroke(rainbowtoggle, 255, 255, 255, 1)
			corner(rainbowtoggle, 1, 0)
			
			local rainbowtogglevisual = Instance.new("Frame", rainbowtoggle) borderpixel(rainbowtogglevisual)
			backgroundcolor(rainbowtogglevisual, 255, 255, 255)
			tbackground(rainbowtogglevisual)
			size(rainbowtogglevisual, 0, 10, 0, 10)
			corner(rainbowtogglevisual, 1, 0)
			center(rainbowtogglevisual)
			pos(rainbowtogglevisual, .5, 0, .5, 0)
			
			local raindowtitle = Instance.new("TextLabel", framePicker) borderpixel(raindowtitle)
			text(raindowtitle, "Rainbow")
			textColor(raindowtitle, 255, 255, 255)
			textFont(raindowtitle, Enum.Font.Cartoon)
			textSize(raindowtitle, 17)
			textX(raindowtitle, Enum.TextXAlignment.Left)
			size(raindowtitle, 0, 14, 0, 14)
			tbackground(raindowtitle)
			pos(raindowtitle, 0, 27, 0, 144)
			
			local ranbowspeedtitle = Instance.new("TextLabel", framePicker) borderpixel(ranbowspeedtitle)
			text(ranbowspeedtitle, "Rainbow Speed")
			textColor(ranbowspeedtitle, 255, 255, 255)
			textFont(ranbowspeedtitle, Enum.Font.Cartoon)
			textSize(ranbowspeedtitle, 17)
			textX(ranbowspeedtitle, Enum.TextXAlignment.Left)
			size(ranbowspeedtitle, 0, 14, 0, 14)
			tbackground(ranbowspeedtitle)
			pos(ranbowspeedtitle, 0, 8, 0, 168)
			
			local bar = Instance.new("Frame", framePicker) borderpixel(bar)
			size(bar, 0, 100, 0, 6)
			pos(bar, 1, -106, 0, 174)
			corner(bar, 1, 0)
			backgroundcolor(bar, 0, 0, 0)
			...
			btn.MouseButton1Click:Connect(function()
				if framePicker.Size == UDim2.new(1, 0, 0, 0) then
					framePicker.Visible = true
					t:Create(framePicker, info, {Size = UDim2.new(1, 0, 0, 190)}):Play()
					t:Create(SectionPage, info, {CanvasSize = SectionPage.CanvasSize+UDim2.new(0, 0, 0, 190)}):Play()
					t:Create(frame1, info, {Size = UDim2.new(1, 0, 0, 190+26)}):Play()
				else
					t:Create(framePicker, info, {Size = UDim2.new(1, 0, 0, 0)}):Play()
					t:Create(SectionPage, info, {CanvasSize = SectionPage.CanvasSize-UDim2.new(0, 0, 0, 190)}):Play()
					t:Create(frame1, info, {Size = UDim2.new(1, 0, 0, 26)}):Play()
					wait(0.4)
					if framePicker.Size == UDim2.new(1, 0, 0, 0) then
						framePicker.Visible = false
					end
			...
			local initialXScale = (0.003 - 0.001) / (0.02 - 0.001)
			fillbar.Size = UDim2.new(initialXScale, 0, 1, 0)
			...
			rainbowtoggle.MouseButton1Click:Connect(function()
				rainbowdb = not rainbowdb
				if rainbowtogglevisual.BackgroundTransparency == 1 then
					t:Create(rainbowtogglevisual, info3, {BackgroundTransparency = 0}):Play()
					for i,v in pairs(textboxs) do
						v.TextEditable = false
					end
				else
					t:Create(rainbowtogglevisual, info3, {BackgroundTransparency = 1}):Play()
					for i,v in pairs(textboxs) do
						v.TextEditable = true
					end
				end
			...
			local gradientborder2 = Instance.new("UIGradient", border2)
			gradientborder2.Transparency = NumberSequence.new{
			...
			local isMouseDown2 = false

			PC.InputBegan:Connect(function(input)
				if input.UserInputType == Enum.UserInputType.MouseButton1 then
					local mouseX, mouseY = input.Position.X, input.Position.Y
					local buttonX = PC.AbsolutePosition.X
					local buttonY = PC.AbsolutePosition.Y
					local buttonWidth = PC.AbsoluteSize.X
					local buttonHeight = PC.AbsoluteSize.Y
			...
			darkPC.InputBegan:Connect(function(input)
				if input.UserInputType == Enum.UserInputType.MouseButton1 then
					local mouseX, mouseY = input.Position.X, input.Position.Y
					local buttonX = darkPC.AbsolutePosition.X
					local buttonY = darkPC.AbsolutePosition.Y
					local buttonWidth = darkPC.AbsoluteSize.X
					local buttonHeight = darkPC.AbsoluteSize.Y
			...
			PC.InputEnded:Connect(function(input)
				if input.UserInputType == Enum.UserInputType.MouseButton1 then
					isMouseDown = false
					isMouseDown2 = false
				end
			...
			darkPC.InputEnded:Connect(function(input)
				if input.UserInputType == Enum.UserInputType.MouseButton1 then
					isMouseDown = false
					isMouseDown2 = false
				end
				...
				while true do
					...
					if not rainbowdb then
						if isMouseDown then
							...
							local posx, posy = mouse.X-PCPos.X, mouse.Y-PCPos.Y
							local PCSize = PC.AbsoluteSize
							if posx >= PCSize.X then posx = PCSize.X end
							if posx <= 0 then posx = 0 end
							if posy >= PCSize.Y then posy = PCSize.Y end
							...
							textboxs[1].Text = tostring(math.floor(color.R*255))
							textboxs[2].Text = tostring(math.floor(color.G*255))
							textboxs[3].Text = tostring(math.floor(color.B*255))
							visualcolor.BackgroundColor3 = color
							callback(color)
						end

						if isMouseDown2 then
							local PCPos = darkPC.AbsolutePosition
							local posx, posy = mouse.X-PCPos.X, mouse.Y-PCPos.Y
							local PCSize = darkPC.AbsoluteSize
							if posx >= PCSize.X then posx = PCSize.X end
							if posx <= 0 then posx = 0 end
							if posy >= PCSize.Y then posy = PCSize.Y end
							if posy <= 0 then posy = 0 end
							circledarkPC.Position = UDim2.new(0, PCSize.X/2, 0, posy)
						...
						rainbowtimer += rainbowSpeed
						color = Color3.fromHSV(rainbowtimer, 1, 1)
						circlePC.Position = UDim2.new(rainbowtimer, 0, 0, 0)
						circledarkPC.Position = UDim2.new(0.5, 0, 0, 0)
						visualcolor.BackgroundColor3 = color
						textboxs[1].Text = tostring(math.floor(color.R*255))
						textboxs[2].Text = tostring(math.floor(color.G*255))
						textboxs[3].Text = tostring(math.floor(color.B*255))
						callback(color)
						if rainbowtimer >= 1 then
							rainbowtimer = 0
						end
					end
				end
			...
			return {
				Update = function(newcolor)
					...
					s = s2
					v = v2
					circlePC.Position = UDim2.new(h2, 0, 0, 0)
					circledarkPC.Position = UDim2.new(0.5, 0, 1-v2, 0)
					visualcolor.BackgroundColor3 = newcolor
					textboxs[1].Text = tostring(math.floor(color.R*255))
					textboxs[2].Text = tostring(math.floor(color.G*255))
					textboxs[3].Text = tostring(math.floor(color.B*255))
					callback(newcolor)
